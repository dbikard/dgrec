...
# %% auto 0
__all__ = ['get_UMI_genotype', 'correct_UMI_genotypes', 'genotype_UMI_counter', 'get_genotypes', 'dgrec_genotypes']
...
def correct_UMI_genotypes(UMI_gencounter: dict, #the output of the get_UMI_genotype function
                          reads_thr=2 #only assign a genotype to a UMI if we have reads_thr reads for that genotype or more
                          ) -> dict:
    ...
    for umi in UMI_gencounter:
        ...
        if n>=reads_thr: #only assign a genotype to a UMI if we have reads_thr reads for that genotype or more
...
def dgrec_genotypes(fastq, ref, umi_size, quality_threshold, ignore_pos, reads_thr, save_umi_data, output):
